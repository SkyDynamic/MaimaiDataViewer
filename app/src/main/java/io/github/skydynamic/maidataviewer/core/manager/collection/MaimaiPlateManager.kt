...
class MaimaiPlateManager(
...
) : MaimaiResourceManager, CollectionManager<MaimaiPlateData> {
    override var _isLoaded: Boolean = false
    override var _currentCollectionVersion: MaiVersion = MaiVersion(-1, 0)

    val assetsUrl = "https://assets2.lxns.net/maimai/plate"

    private var plateData: Map<Int, MaimaiPlateData> = emptyMap()
    ...
    override fun getCollection(id: Int): MaimaiPlateData? {
        return plateData[id]
    }

    override fun search(
        keyword: String,
        filterAction: ((List<MaimaiPlateData>) -> List<MaimaiPlateData>)?
    ): List<MaimaiPlateData> {
